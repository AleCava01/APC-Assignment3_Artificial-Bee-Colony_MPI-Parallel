...
void abc_algorithm(const int & num_bees, 
    ...
    // Pulizia file CSV (solo il rank 0 all'inizio)
    if (rank == 0) {
        std::ofstream ofs("bees_positions.csv", std::ios::trunc);
        ofs.close();
    }
    ...
    for (std::size_t iter = 0; iter < iter_num; ++iter){
        ...
        // --- ESPORTAZIONE DATI PER GRAFICO (Ogni 5 iterazioni) ---
        if (iter % 1 == 0) {
            // 1. Trova il miglior valore locale in questo momento
            double local_min = *std::min_element(local_best_OF_value.begin(), local_best_OF_value.end());
            double global_best_so_far;

            // 2. Il Rank 0 raccoglie il minimo assoluto tra tutti i rank
            MPI_Reduce(&local_min, &global_best_so_far, 1, MPI_DOUBLE, MPI_MIN, 0, MPI_COMM_WORLD);

            // 3. Raccogliamo le posizioni come prima
            std::vector<double> all_coords(num_bees * dim);
            std::vector<double> local_flat_coords;
            for(int i=0; i < local_num_bees; ++i) {
                for(int d=0; d < dim; ++d) local_flat_coords.push_back(local_current_positions[i][d]);
            }

            MPI_Gather(local_flat_coords.data(), local_num_bees * dim, MPI_DOUBLE,
                    all_coords.data(), local_num_bees * dim, MPI_DOUBLE, 0, MPI_COMM_WORLD);

            if (rank == 0) {
                std::ofstream outfile("bees_positions.csv", std::ios::app);
                for (int b = 0; b < num_bees; ++b) {
                    outfile << iter << "," << b;
                    for (int d = 0; d < dim; ++d) outfile << "," << all_coords[b * dim + d];
                    // Aggiungiamo il miglior valore globale come ultima colonna
                    outfile << "," << global_best_so_far << "\n";
                }
                outfile.close();
            }
        }
